...
- 2026-05-21T00:00Z [PLANNED] capture-set-cookie — GH-45, part of Cookie Support epic (#43). Capture `Set-Cookie` from responses into the cookie jar. Proxy Edge Function returns `setCookies: string[]` via `getSetCookie()`; new pure `extractSetCookies(result)` helper in cookies.js (proxy field / Tauri header entries / browser empty); `setCookiesFromResponse` removes a cookie on already-expired Set-Cookie (Max-Age=0); execution hooks wire capture via `resolvedUrl`. No Rust change (Tauri already returns set-cookie separately). Browser direct path cannot capture (forbidden header).
