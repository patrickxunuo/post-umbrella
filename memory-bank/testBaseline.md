...
- [x] cookies-tab — Response viewer Cookies tab surfaces cookies set by a response (GH-47), incl. GH-57 value-column truncation/reveal — **executed & passing** (2026-05-22, real local Supabase + Vite dev server): `e2e/cookies-tab.spec.ts` now has 7 chromium tests, all green — tab-appears-and-lists-cookie (via `httpbin.org/response-headers?Set-Cookie=...` through the proxy `setCookies` path), tab-hidden-without-set-cookie, switching-tabs-preserves-response, tab-hidden-in-example, plus 3 GH-57 value-cell tests (truncates-and-toggles-expanded-on-click, has-testid-and-is-truncated-by-default, reveals-by-wrapping-on-click-and-collapses-back). Core logic (`getResponseCookies`) also covered by Vitest unit cases.
- [x] cookie-manager — Cookie Manager dialog + Auth-tab entry point (GH-48), incl. GH-57 tag/editor layout restructure — **executed & passing** (2026-05-22, real local Supabase + Vite dev server): `e2e/cookie-manager.spec.ts` 2 chromium tests green — full lifecycle (open via Auth-tab `open-cookie-manager` → add domain → add/edit/remove cookie → persistence across reload → search empty-state → remove domain) and the GH-57 structural test (`value editor is a single shared element rendered outside cookie tags`: editor is never a descendant of `[data-testid="cookie-tag"]`, exactly one editor in DOM, tag count unchanged when editor opens). Pure helpers (`filterDomains`, `makeCookie`) also covered by Vitest.
- [x] curl-cookie — cURL cookie round-trip: export emits `-b`, import parses `-b`/`--cookie`/`Cookie:` header into one Cookie header (GH-49) — **no E2E needed; covered by Vitest**: pure string transformation in `generateCurl` (export) + `parseCurl` (import), no backend interaction and no new user-facing flow beyond the existing `import-curl` E2E (`e2e/import.spec.ts`). Fully covered by `src/components/curlCookies.test.js` (34 cases: `-b` emission, no `-b`/`-H Cookie:` duplication, manual-wins merge, import of `-b`/`--cookie`/`Cookie:` header, multi-source merge, and export→import round-trip). Note: `vitest.config.js` gained `test.env` dummy Supabase creds so component-importing unit tests load without a backend.
