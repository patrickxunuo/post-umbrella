...
- [ ] cookie-response-tab — Response viewer Cookies tab shows cookies set by a response (GH-47) — **E2E deferred (not yet verified locally)**: GH-47 adds the first cookie UI surface (read-only Cookies tab in `ResponseViewer`), with `data-testid` selectors (`response-tab-cookies`, `response-cookies`, `cookie-row`) ready for E2E targeting. A spec was drafted (send via proxy to `https://httpbin.org/response-headers?Set-Cookie=...`, assert the tab appears and lists the cookie; assert a no-cookie response hides the tab) but **not committed** because the cookie-row derivation (`getResponseCookies`) is fully covered by Vitest unit tests and the E2E could not be run/verified in the dev environment (no Supabase backend + auth storage state available). Add the verified E2E once a local backend + `e2e/.auth/user.json` are available.
