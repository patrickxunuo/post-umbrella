...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    if (listError) {
      return new Response(
        JSON.stringify({ message: "Failed to find user" }),
    ...
    if (!targetUser) {
      return new Response(
        JSON.stringify({ message: "User not found. They must be invited through User Management first." }),
        { status: 404, headers: { ...corsHeaders, "Content-Type": "application/json" } }
      );
    }

    // Check if user has a profile (meaning they were properly invited)
    ...
    if (!targetProfile) {
      return new Response(
        JSON.stringify({ message: "User has not been invited yet. Please invite them through User Management." }),
        { status: 400, headers: { ...corsHeaders, "Content-Type": "application/json" } }
      );
    }
